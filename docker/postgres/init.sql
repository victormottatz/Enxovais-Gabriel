-- ==============================================================================
-- Enxovais Gabriel - Gestão Comercial, Estoque e Crediário Próprio (Fichas)
...
-- 2. Tabela: Produtos (Catálogo de Utilidades Domésticas & Enxovais)
CREATE TABLE IF NOT EXISTS produtos (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    codigo_sku VARCHAR(50) UNIQUE,
    nome VARCHAR(150) NOT NULL,
    descricao TEXT,
    categoria VARCHAR(50) NOT NULL DEFAULT 'OUTROS', -- 'CAMA_MESA_BANHO', 'COZINHA', 'DECORACAO', 'ORGANIZACAO', 'OUTROS'
...
-- 3. Tabela: Fichas de Crediário (Conta Corrente do Cliente / Dividendo Acumulado)
...
CREATE INDEX IF NOT EXISTS idx_fichas_cliente_id ON fichas_crediario (cliente_id);
CREATE INDEX IF NOT EXISTS idx_fichas_dia_vencimento ON fichas_crediario (dia_vencimento_padrao);
CREATE INDEX IF NOT EXISTS idx_fichas_status ON fichas_crediario (status_ficha);
...
-- 6. Tabela: Encomendas (Itens solicitados a Distribuidores/Fabricantes)
...
CREATE INDEX IF NOT EXISTS idx_encomendas_status ON encomendas (status_encomenda);
CREATE INDEX IF NOT EXISTS idx_encomendas_data_previsao ON encomendas (data_previsao_chegada);

-- 7. Tabela: Movimentações da Ficha (Extrato / Auditoria do Crediário)
...
CREATE INDEX IF NOT EXISTS idx_movimentacoes_ficha_id ON movimentacoes_ficha (ficha_id);

-- 8. Tabela: Parcelas de Crediário
...
CREATE INDEX IF NOT EXISTS idx_parcelas_ficha_id ON parcelas_crediario (ficha_id);
CREATE INDEX IF NOT EXISTS idx_parcelas_vencimento ON parcelas_crediario (data_vencimento);
CREATE INDEX IF NOT EXISTS idx_parcelas_status ON parcelas_crediario (status);

-- 9. Tabela: Configurações Gerais e Mensageria
...
-- 10. Tabela: Logs de Mensagens do WhatsApp
CREATE TABLE IF NOT EXISTS logs_mensagens (
    ...
    tipo_mensagem VARCHAR(50) NOT NULL, -- 'VENDA_CREDIARIO', 'LEMBRETE_PAGAMENTO', 'RECIBO_PAGAMENTO', 'ENCOMENDA_CHEGOU', 'AVULSO'
    status_envio VARCHAR(20) NOT NULL, -- 'ENVIADO', 'ERRO', 'FILA'
    detalhes_erro TEXT,
